...
const SPOTLIGHT_VISIBLE_STATES = new Set(['CURRENT', 'REVIEW-DUE']);
...
for (const record of data.records ?? []) {
  ...
  if (!SPOTLIGHT_VISIBLE_STATES.has(record.publication_state)) {
...
console.log(`Training opportunities structured-data validation passed: ${data.records.length} record(s) (${summary}); Learning Spotlight held by exactly one record (${spotlightHolders[0]}); corpus matches the ${corpus.record_ids.length} record(s) declared in automation/training-opportunities/canonical-corpus.json and every one is generator-owned on ${(corpus.governed_surfaces ?? []).join(' + ')}.`);

...
for (const record of data.records ?? []) {
  ...
  if (!expired && !indexHtml.includes(`href="${record.detail_page}"`)) errors.push(`${label}: current detail route missing from Home markup`);
  if (!expired && record.card_action?.url && !indexHtml.includes(record.card_action.url)) errors.push(`${label}: current Home action URL missing from Home markup`);
  if (!expired && record.media?.asset && !indexHtml.includes(record.media.asset)) errors.push(`${label}: current media asset missing from Home markup`);

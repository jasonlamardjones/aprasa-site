...
//
// Usage:
//   node scripts/write-control-plane-result.mjs --draft=<path-to-draft.json> [--ref=<results-ref>] [--root=<repo-root>]
//
...
// This never pushes, merges, deploys, or grants any authority.
//
// Retrying the same logical draft — the same content, at a later instant —
// is a no-op: result_id excludes created_at, so identity is stable across
// retries and a rerun reports ALREADY_PRESENT rather than minting a second
// record.
...
if (!draftArg) {
  console.error('Usage: node scripts/write-control-plane-result.mjs --draft=<path-to-draft.json> [--ref=<results-ref>] [--root=<repo-root>]');
  process.exit(2);
}
...
try {
  ...
  const options = refArg ? { ref: refArg.slice('--ref='.length) } : {};
  const outcome = writeResult(ROOT, record, options);
